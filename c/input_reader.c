...
int run_input_reader(char* input_path, char* cr3_path, bool input_is_pipe) {
	...
	while(true) {
		...
		if(next_event_id == TRACE_EVENT_ID_READ || next_event_id == TRACE_EVENT_ID_WRITE) {
			...
			//TODO send to coordinator using mpi
		...
		if((cache_access_index != 0) &&(cache_access_index % BATCH_SIZE == 0)) {
			int buffer_index = (cache_access_index / BATCH_SIZE) - 1;
			...
			send_success = MPI_Isend(&access_buffer[(buffer_index*(BATCH_SIZE)) +cache_access_index], 
						 BATCH_SIZE, mpi_access_datatype, 
		...
		cache_access_index = ((cache_access_index+1) % (2*BATCH_SIZE));
