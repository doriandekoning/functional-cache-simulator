...
int main(int argc, char** argv) {
	...
	char *pg_tables_dump_path, *cr3_output_path, *input_path, *initial_cr_values_path;
  ...
  debug_printf("%s, %d\n", argv[1],  strcmp(argv[1], "y"));
	...
	if(read_pgtables) {
		...
		pg_tables_dump_path = argv[4];
		cr3_output_path = argv[5];
		initial_cr_values_path = argv[6];
    if(world_rank == 0) {
    ...
    debug_printf("Input is %s", input_is_file ? "file" : "pipe");
		...
		run_input_reader(input_path, cr3_output_path, read_pgtables, input_is_file);
	}else {
