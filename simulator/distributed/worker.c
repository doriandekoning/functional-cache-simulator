...
// 		return err;
// 	}
// 	printf("Allocating a state array of size:%d * %lu\n", AMOUNT_SIMULATED_PROCESSORS * (CACHE_AMOUNT_LINES/amount_workers)* (1<<CACHE_LINE_SIZE_BITS), sizeof(struct CacheEntry*)); //TODO check if size is correct
// 	int* cr3_values = calloc(AMOUNT_SIMULATED_PROCESSORS, sizeof(uint64_t));
// 	CacheState states = calloc(AMOUNT_SIMULATED_PROCESSORS * (CACHE_AMOUNT_LINES/amount_workers) * (1<<CACHE_LINE_SIZE_BITS), sizeof(struct CacheEntry*));
// 	if(states == NULL) {
...
// 			CacheSetState set_state = get_cache_set_state(states, msg->address, msg->cpu);
// 			CacheEntryState entry_state = get_cache_entry_state(set_state, msg->address);
...
// 			// 			// new = check_eviction(cache_set_state); //TODO should be done in cache.c
// 			// 			set_cache_set_state(states, new, msg->address, msg->cpu);
// 			// 		}
// 			// 	}
// 			// }
