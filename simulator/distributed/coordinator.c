...
int run_coordinator(int world_size, bool read_pgtables, char* input_pagetables, char* cr_values_path) {
  ...
  uint64_t control_registers[AMOUNT_SIMULATED_PROCESSORS][5];
	...
	cache_access accesses[sizeof(cache_access)*INPUT_READER_BATCH_SIZE];
	...
	start = clock();
  	...
  	while(true){
    	...
    	if(MPI_Recv(accesses, INPUT_READER_BATCH_SIZE, mpi_access_type, 1, 0, MPI_COMM_WORLD, &status) != MPI_SUCCESS) {
	    ...
	    for(int i = 0; i < count; i++) {
			accesses[i].cpu = map_cpu_id(accesses[i].cpu);
      		if(accesses[i].type == CACHE_READ || accesses[i].type == CACHE_WRITE){
				if(read_pgtables && total_mem_access == 0){
				  debug_print("Reading initial cr values!\n");
					if(read_cr_values(cr_values_path, control_register_values)) {
				...
				if(paging_enabled(control_register_values, accesses[i].cpu)) {
					accesses[i].address = vaddr_to_phys(mem, control_registers[accesses[i].cpu][3], accesses[i].address, false);
				}else{
					phys_mem_access++;
				}
				if(total_mem_access % 1000000 == 0){
				  printf("%lu Mil out of %lu Mil accesses not mappable: %f\%\n", unable_to_map / 1000000, total_mem_access / 1000000, (double)unable_to_map/(double)total_mem_access*100.0);
				}
				if(accesses[i].address == NOTFOUND_ADDRESS ){
          		debug_printf("unable to map: 0x%016lx with cr: 0x%016lx\n", accesses[i].address, control_registers[accesses[i].cpu][3]);
					unable_to_map++;
          			exit(1);
				}
				//From here addresses[i].address contains the physical address
				if(accesses[i].type == CACHE_WRITE) {
          			debug_printf("Writing to 0x%016lx the value 0x%016\n", accesses[i].address, accesses[i].data);
				...
				if(total_mem_access %10000000 == 0) {
			...
			//CR CHANG
				...
				bool before_paging_enabled = paging_enabled(control_register_values, accesses[i].cpu);
				control_registers[accesses[i].cpu][accesses[i].size] = accesses[i].address;
				if(accesses[i].size == 0 && before_paging_enabled != paging_enabled(control_register_values, accesses[i].cpu)){
					printf("Paging %sabled!\n", paging_enabled(control_register_values, accesses[i].cpu) ? "en" : "dis");
				}
			#endif /* VIRT_TO_PHYS_TRANSLATION*/
			}else{
