...
uint64_t vaddr_to_phys(struct Memory* mem, uint64_t l1_phys, uint64_t virtaddress, bool debug, bool* is_user_page, bool* cache_disable) {
	if(debug){
	...
	if(l2_phys & PG_PSE_MASK) {
		...
		*cache_disable = !!(l2_phys & PG_CACHE_DISABLE_MASK);
